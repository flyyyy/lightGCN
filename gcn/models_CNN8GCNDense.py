...
class BjyGCN(BjyModel):
    ...
    def _build(self):
        ...
        self.CNN_layers.append(tf.layers.MaxPooling2D(pool_size = 2, strides = 2))
        self.CNN_layers.append(tf.layers.Conv2D(filters = 384, kernel_size = 3, padding = 'same', activation=tf.nn.relu))
        #self.CNN_layers.append(tf.layers.Dense(units=512, activation=tf.nn.relu))
